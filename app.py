...
def expand_bp_labs(labs):
    ...
    ให้แยกเป็น Systolic/Diastolic แทนที่ค่ารวมเดิม (ไม่แสดงค่ารวมซ้ำในตาราง
    เพราะไม่มี atom ให้ resolve สถานะ จึงจะขึ้น "-" เสมอถ้าคงไว้)
    ถ้าแยกไม่ได้ (รูปแบบไม่ตรง) จะคงค่าดิบเดิมไว้ตามปกติ
    """
    if not labs:
        return labs
    ...
    for header, value in labs.items():
        ...
        split_ok = False
        sbp_text = dbp_text = None

        if normalized_header in BP_HEADER_ALIASES and "/" in value_text:
